...
<main class="pt-90">
    ...
    <section class="shop-checkout container">
      ...
      <div class="shopping-cart">
        ...
        <div class="cart-table__wrapper">
                ...
                <tr>
                    ...
                    <td>
                      ...
                      <form method="POST" action="{{route('wishlist.move.to.cart',['rowId'=>$item->rowId])}}">
                        @csrf
                        <button type="submit" class="btn btn-sm btn-warning" style="border-radius:10px;">Move to Cart</button>
                      </form>
                    </div>
                    <div class="col-6">
          ...
          <div class="cart-table-footer">
           <form method='POST' action="{{route('wishlist.item.clear')}}">
            @csrf
            @method('DELETE')
                <button type="submit" class="btn btn-light" style="border-radius: 10px; background-color:brown; color:white; ">CLEAR WISHLIST</button>
            </form>
          </div>
        </div>
        @else
        <div class="row">
            <div class="col-md-12 ">
                <p style="font-size: 20px; color:red;">No Item Found In Your Wishlist</p>
                <a href="{{route('shop.index')}}" class="btn btn-info" style="border-radius:10px; font-bold">Wishlist Now</a>
            </div>
        </div>
        @endif
